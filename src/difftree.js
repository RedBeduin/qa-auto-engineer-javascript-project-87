...
const diff = (objFirst, objSecond) => {
  ...
  for (const key of keys) {
    ...
    (
      ...
      result.push({ key, obj: objSecond[key], action: 'added' })
    }
    else if
    (
      objFirst[key] !== objSecond[key]
      ...
      result.push({ key, obj: objFirst[key], action: 'removed' })
    }
    else if
    (
      objFirst[key] !== objSecond[key]
      ...
      result.push({ key, objFirst: objFirst[key], objSecond: objSecond[key], action: 'updated' })
    }
    else if
    (objFirst[key] === objSecond[key]) {
